...
var creatHtml = function () {
    ...
    node.id = option.id
    node.innerHTML = `
      <div class="my-el-message ${option.type}">
        <div class="el-message-cover">
        </div>
        <div class="my-el-message__group">
          <p id="${option.id}-p">${option.message}</p>
          <div class="my-el-message-close" id="${option.id}-close">
          </div>
        </div>
      </div>`
...
const modalInfoArray = ['success', 'warning', 'info', 'error']
modalInfoArray.forEach(function (type) {
